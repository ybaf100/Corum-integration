...
On Android, v0.2.37 keeps this capture inside Geometry Dash's Cocos renderer using `CCRenderTexture`; it does not invoke Android `MediaProjection` or attempt to capture other apps/system UI. The mod therefore does not request Android's screen-recording consent prompt for End Screen evidence. The render target uses Cocos logical dimensions so the engine applies its content scale exactly once, while PNG compression and file I/O run off the render thread. Capture waits for the End Level entrance animation, and a concurrent evidence upload no longer suppresses captures from other maps. Pending metadata is persisted per account and canonical map before encoding, and completed staged PNGs can be recovered after restarting the game. Android32 and Android64 are built as native Geode targets and can be combined with the Windows build into one package.
