...
On Android, v0.2.34 keeps this capture inside Geometry Dash's Cocos renderer using `CCRenderTexture`; it does not invoke Android `MediaProjection` or attempt to capture other apps/system UI. The mod therefore does not request Android's screen-recording consent prompt for End Screen evidence. Android32 and Android64 are built as native Geode targets and can be combined with the Windows build into one package.
