# Corum Integration 기록 검증 데이터 안내

Corum Integration을 설치하거나 사용하기 전에 아래 내용을 확인해 주세요.

## 처리하는 정보

기록 제출 기능이 활성화된 경우 모드는 다음 정보를 처리할 수 있습니다.
...
- 사용자가 종이비행기 창의 `Submit` 또는 메인 메뉴의 `Submit All`을 누른 경우에만 기록 데이터와 사용 가능한 증거가 서버로 전송됩니다.
...
- Geode 설정에서 `Record submission`을 끄면 기록 제출 UI와 새로운 End Level 검증 이미지 준비가 비활성화됩니다.
